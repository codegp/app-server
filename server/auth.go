...
func loginHandler(w http.ResponseWriter, r *http.Request) *requestError {
...
func oauthCallbackHandler(w http.ResponseWriter, r *http.Request) *requestError {
	...
	if err != nil {
		return requestErrorf(err, "could not fetch Google profile: %v", err)
	}
...
func logoutHandler(w http.ResponseWriter, r *http.Request) *requestError {
	session, err := sessionStore.New(r, defaultSessionID)
	if err != nil {
		return requestErrorf(err, "could not get default session: %v", err)
	}
